...
const proxy_1 = require("@actions/http-client/lib/proxy");
...
async function downloadAndExtractZstdWithStreaming(codeqlURL, authorization, headers, tarVersion, logger) {
    ...
    let downloadUrl = adjustUrlByProxy(codeqlURL) || codeqlURL;
    headers = Object.assign({ "User-Agent": "CodeQL Action" }, authorization ? { authorization } : {}, headers);
    const response = await new Promise((resolve) => follow_redirects_1.https.get(downloadUrl, {
        headers,
...
function adjustUrlByProxy(url) {
    const proxyUrl = (0, proxy_1.getProxyUrl)(new URL(url));
